...
def copy_git_hooks(git_hooks_dir: str):
    """Copy git hook scripts to the git_hooks directory."""
    git_hooks_src_dir = files('hooked.data.git_hooks')
    with as_file(git_hooks_src_dir) as git_hooks_src:
        for root, dirs, _files in os.walk(git_hooks_src):
            ...
            dest_dir = os.path.join(git_hooks_dir, rel_path)
...
def copy_git_hook_templates(git_template_dir: str):
    """Copy git hook template scripts to the git_template directory."""
    git_template_src_dir = files('hooked.data.git_hook_templates')
    with as_file(git_template_src_dir) as git_hook_templates_src:
        for root, dirs, _files in os.walk(git_hook_templates_src):
            rel_path = os.path.relpath(root, git_hook_templates_src)
            dest_dir = os.path.join(git_template_dir, rel_path)
            os.makedirs(dest_dir, exist_ok=True)
            for file in _files:
                src_file_path = os.path.join(root, file)
                dst_file_path = os.path.join(dest_dir, file)
